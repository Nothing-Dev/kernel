			    ...
			    void (*getevent) (void *ptr, size_t len))
{
	...
	case AUDPP_MSG_CFG_MSG:
		if (msg[0] == AUDPP_MSG_ENA_ENA) {
			MM_INFO("ENABLE %d audpp->enabled id =%d\n",audpp->enabled,id);
			if (!audpp->enabled) {
				audpp->enabled = 1;
				audpp_broadcast(audpp, id, msg);
			} else {
				cid = msg[1];
				MM_INFO("Fake enable cid %d \n",cid);
...
int audpp_enable(int id, audpp_event_func func, void *private)
{
	...
	LOG(EV_ENABLE, 1);
	MM_INFO("audpp open count %d enable %d\n",audpp->open_count,audpp->enabled);
	if (audpp->open_count++ == 0) {
		MM_INFO("audpp_enable\n");
